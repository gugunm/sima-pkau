      ...
      <template #table-row="props">
        <template v-if="props.column.field == 'actions'">
          ...
          <CButton
            ...
            @click="
              $router.push({
                ...
                params: { idKontributor: props.row.idKontributor },
              })
            "
            class="mx-1"
            >Edit</CButton
          ...
          <CButton
            ...
            size="sm"
            variant="outline"
            @click="resDelAction(props.row.idKontributor)"
            class="mx-1"
            >Hapus</CButton
          >
        </template>
...
const columns = [
  ...
  {
    label: 'Unit Kerja Kontributor',
    field: 'unitKontributor',
    thClass: 'text-sm',
    tdClass: 'text-sm',
  },
  {
    ...
    async actionDelete() {
      try {
        const response = await axios({
          method: 'DELETE',
          baseURL: this.$apiAddress,
          url: `/api/kontributor/${this.idToDel}`,
          params: {
            token: localStorage.getItem('token'),
